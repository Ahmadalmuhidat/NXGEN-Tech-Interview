...
export default function Home() {
  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      const devicesResponse = await AxiosClient.get('/devices');
      const devices = Array.isArray(devicesResponse.data) ? devicesResponse.data : devicesResponse.data?.data || [];

      let totalDataPoints = 0;
      if (devices.length > 0) {
        try {
          const dataResponse = await AxiosClient.get('/devices/data/count/last24h');
          totalDataPoints = dataResponse.data?.data || 0;
        } catch (error) {
          console.error('Error fetching total data points:', error);
          totalDataPoints = 0;
        }
      }
