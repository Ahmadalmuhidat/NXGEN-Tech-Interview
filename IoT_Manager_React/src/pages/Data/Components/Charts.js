...
// Area Chart - Filled version of line chart
export function AreaChart({ data, maxValue, height = 200 }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-card p-6 border border-gray-100">
      <h3 className="text-lg font-semibold text-gray-900 mb-6">Area Chart</h3>
...
// Bar Chart - For comparing time periods
export function BarChart({ data, maxValue, height = 200 }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-card p-6 border border-gray-100">
      <h3 className="text-lg font-semibold text-gray-900 mb-6">Bar Chart</h3>
      <div className="relative" style={{ height: `${height}px` }}>
        <svg className="w-full h-full" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none">
          <defs>
...
export function ScatterPlot({ data, maxValue, height = 200 }) {
  if (!data || data.length === 0) return null;
  ...
  return (
    <div className="bg-white rounded-2xl shadow-card p-6 border border-gray-100">
      <h3 className="text-lg font-semibold text-gray-900 mb-6">Scatter Plot</h3>
      <div className="relative" style={{ height: `${height}px` }}>
        <svg className="w-full h-full" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none">
          {/* Grid lines */}
          {[0, 25, 50, 75, 100].map((percent) => (
          ...
          {data.map((d, i) => {
            ...
            // Size based on value (larger = higher value)
            ...
            return (
              <circle
                ...
                fill={color}
                opacity="0.7"
                className="hover:opacity-100 hover:r-8 transition-all"
              />
            );
          })}
        </svg>
...
export function Sparkline({ data, maxValue, height = 40 }) {
  if (!data || data.length === 0) return null;
